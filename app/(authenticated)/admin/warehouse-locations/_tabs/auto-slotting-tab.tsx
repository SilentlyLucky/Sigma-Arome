...
export function AutoSlottingTab() {
  ...
  useEffect(() => {
    fetch(
      '/api/items/batches?filter[status][_in]=approved,storage_assigned,stored_available' +
        '&fields[]=id&fields[]=batch_number&fields[]=status&fields[]=material_id.name&sort[]=-date_created&limit=200',
      { cache: 'no-store' }
    )
      .then((r) => r.json())
      .then((d) => {
      ...
      {result && !computing && (
        <Stack gap="md">
          <Group gap="xs">
            ...
            {result.material && <Text size="sm" c="dimmed">{result.material.name}</Text>}
          </Group>
              ...
              {result.candidates.map((c) => (
                ...
                >
                  ...
                  <Group gap="lg" wrap="wrap">
                    <ScoreBit label="Temperature" value={c.temperature_score} max={40} />
                    <ScoreBit label="Hazard" value={c.hazard_score} max={30} />
                    <ScoreBit label="Capacity" value={c.capacity_score} max={15} />
                    <ScoreBit label="Occupancy" value={c.occupancy_score} max={15} />
                  </Group>
...
function ScoreBit({ label, value, max }: { label: string; value: number; max: number }) {
  ...
  return (
    <div style={{ minWidth: 110 }}>
      ...
      <Progress value={pct} size="sm" mt={2} color={pct >= 75 ? 'teal' : pct >= 40 ? 'yellow' : 'red'} />
    </div>
  );
}
