...
/**
 * Create BOM / Formula — custom form.
 * BOM name = Product name (merged into one field).
 * The product dropdown acts as the BOM name selector.
 */
...
export default function CreateBOMPage() {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      // Name is auto-generated by the backend extension from product name + version
  ...
  return (
    <Stack gap="md">
      ...
      <Alert icon={<IconInfoCircle size={16} />} color="blue" variant="light">
        The formula name is automatically derived from the product you select.
        After saving, you can add materials and quantities on the detail page.
      </Alert>

      <Paper p="md" radius="md" withBorder>
        <Stack gap="sm">
          {/* Product = BOM Name (merged field) */}
          <Group grow align="flex-start">
            {loading ? (
              <Input label="Product (Formula Name) *" placeholder="Loading products..." disabled onChange={() => {}} />
              ...
              <SelectDropdown
                ...
                onChange={(v) => setForm((f) => ({ ...f, product_id: String(v ?? '') }))}
                required
              />
            ) : (
              <Input label="Product (Formula Name) *" value="" placeholder="No active products" disabled onChange={() => {}} />
            )}
            <Input
              label="Version *"
