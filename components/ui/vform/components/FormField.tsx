...
import { getAutoGenerationType, isFieldReadOnly, isNewItem, getFieldDisplayName } from '@/lib/buildpad/utils';
...
function getGeneratedPlaceholder(field: TFormField, displayName: string): string | undefined {
  const special = field.meta?.special ?? [];
  const fieldKey = field.field.toLowerCase();
  const generationType = getAutoGenerationType(field);

  if (special.includes('date-created') || ['date_created', 'created_at'].includes(fieldKey)) {
    return 'Creation date will use the time you save this record';
  }

  if (special.includes('date-updated') || ['date_updated', 'updated_at'].includes(fieldKey)) {
    return 'Last updated date will refresh when this record changes';
  }

  if (special.includes('user-created') || ['user_created', 'created_by'].includes(fieldKey)) {
    return 'Created by will use the current user';
  }

  if (special.includes('user-updated') || ['user_updated', 'updated_by'].includes(fieldKey)) {
    return 'Updated by will use the user who last saves this record';
  }

  if (generationType === 'auto-increment') {
    return `${displayName} will get the next number when saved`;
  }

  if (generationType === 'uuid') {
    return `${displayName} will get a unique ID when saved`;
  }

  if (generationType === 'timestamp') {
    return `${displayName} will use the time you save this record`;
  }

  if (generationType === 'user') {
    return `${displayName} will use the current user`;
  }

  if (fieldKey === 'id' || field.schema?.is_primary_key) {
    return `${displayName} will get a unique ID when saved`;
  }

  if (fieldKey.endsWith('_number') || fieldKey.endsWith('_code')) {
    return `${displayName} will be created when saved`;
  }

  if (field.meta?.readonly || field.schema?.default_value) {
    return `${displayName} will be filled when saved`;
  }
...
export const FormField: React.FC<FormFieldProps> = ({
  ...
  // Get effective value — never use raw schema.default_value directly
  // (it may contain SQL like gen_random_uuid(), ''::character varying, etc.)
  ...
  // For generated readonly fields in create mode, show what the system will fill.
  const autoGeneratedPlaceholder = useMemo(() => {
    if (context !== 'create' || !isDisabled) return undefined;
    return getGeneratedPlaceholder(field, displayName);
  }, [context, field, displayName, isDisabled]);
    ...
    >
      <Stack gap="xs">
        ...
        <FormFieldInterface
          ...
          autoGeneratedPlaceholder={autoGeneratedPlaceholder}
        />
