/**
 * @buildpad-origin @buildpad/utils/utils/field-interface-mapper
 * @buildpad-version 0.2.0
 *
 * This file was copied from Buildpad UI Packages.
 * To update, run: npx @buildpad/cli add utils/field-interface-mapper --overwrite
 *
 * Docs: https://buildpad.dev/components/utils/field-interface-mapper
 */
 ...
 * Only returns true for primary key UUID fields, never for FK/relation fields.
 */
export function isUUIDAutoGenerated(field: Field): boolean {
  const { type, schema, meta } = field;

  // Never treat relation fields as auto-generated
  const special = meta?.special ?? [];
  const isRelationField = Array.isArray(special) && (
    special.includes("m2o") || special.includes("o2m") ||
    special.includes("m2m") || special.includes("m2a")
  );
  if (isRelationField) return false;
  if (schema?.foreign_key_table || schema?.foreign_key_column) return false;
 ...
 * 
 * IMPORTANT: FK/relation fields (M2O, O2M, M2M) with gen_random_uuid() default
 * are NOT auto-generated — they are user-selected foreign keys. Only primary keys
 * and system fields (date-created, user-created, etc.) should be treated as auto-generated.
 */
export function hasAutoGeneratedDefault(field: Field): boolean {
  const { type, schema, meta } = field;

  // ── Guard: Never treat relation fields as auto-generated ──
  // FK fields (material_id, supplier_id, etc.) may have gen_random_uuid() set by DB
  // but they are user-editable foreign keys, not auto-generated values.
  const special = meta?.special ?? [];
  const isRelationField = Array.isArray(special) && (
  ...
  const hasForeignKey = Boolean(
    schema?.foreign_key_table || schema?.foreign_key_column
  );
  if (isRelationField || hasForeignKey) {
    return false;
  }
  ...
  if (
    ...
    // Double-check: only treat as auto-generated if it's a PK or system field
    // Non-PK UUID fields with gen_random_uuid() are likely FK fields with wrong defaults
    if (!schema?.is_primary_key && isUUID) {
      return false;
    }
    return true;
  }
...
export function getFieldDefault(field: Field): unknown {
  ...
  // Filter out database-generated / system defaults that should never be form values
  if (
    // Function calls: gen_random_uuid(), now(), nextval(), uuid_generate_v4()
    ...
    // PostgreSQL type casts: ''::character varying, ''::text, etc.
    defaultValue.includes("::") ||
    // PostgreSQL data types used as values (should never happen but defensive)
    /^(character varying|text|uuid|integer|bigint|boolean|timestamp|jsonb?|float|decimal|numeric)$/i.test(defaultValue.trim())
  ) {
    return undefined;
  }
  ...
  if (!isNaN(Number(defaultValue)) && defaultValue.trim() !== '') {
    return Number(defaultValue);
  }

  // Handle string defaults (remove quotes if present)
  if (defaultValue.startsWith("'") && defaultValue.endsWith("'")) {
    const inner = defaultValue.slice(1, -1);
    // Empty string default → don't pre-fill
